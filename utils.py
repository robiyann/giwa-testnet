...
class MultiAccountFromPK:
    ...
    def get_owlto_hex_data(self):
        ...
        # (pakai bytecode yang kamu berikan)
        return (
    ...
    def get_owlto_erc20_bytecode(self):
        ...
        # (pakai bytecode penuh yang kamu berikan)
        return (
    ...
    def send_transaction_batch(
        ...
        """Kirim transaksi paralel dari banyak akun."""
        results = []
        # baca delay_range dari config default di class (kalau ada)
        delay_min, delay_max = 0.2, 0.8
        try:
            # kalau dipanggil dari main yang punya config manager, user biasanya
            # sudah set delay_range di config.json — tapi di level util ini kita default.
            pass
        except:
            pass
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for account in accounts:
                ...
                time.sleep(random.uniform(delay_min, delay_max))  # hindari spam
    ...
    def _send_single_transaction(self, private_key, from_address, hex_data, gas_limit, line_number):
        """Kirim transaksi TANPA menunggu receipt (mode 'sent')."""
        try:
    ...
    def _send_single_transaction_with_receipt(
        ...
        """Kirim transaksi & TUNGGU receipt (cek status on-chain)."""
        try:
            tx = self._base_tx(from_address, gas_limit, hex_data)
            signed_txn = self.w3.eth.account.sign_transaction(tx, private_key)
            tx_hash = self.w3.eth.send_raw_transaction(signed_txn.raw_transaction)
